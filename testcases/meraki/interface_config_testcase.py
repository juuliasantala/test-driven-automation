...
class InterfaceConfigTestcase(aetest.Testcase):
    ...
    def get_device_interface_details(self, steps, device, interfaces, api_key):
        ...
        with  steps.start(
            ...
            try:
                switch_interfaces = dashboard.switch.getDeviceSwitchPortsStatuses(serial=device)
                self.interfaces = [
                    {
                        "id":interface["portId"],
    ...
    def test_interface_status(self, steps, device):
        ...
        for interface in self.interfaces:
            ...
            with steps.start(
                f"{device}: Interface {interface_id}",
                continue_=True
            ) as step:

                if interface["status"] != "Connected":
                    step.failed(f"{device} {interface_id} is 💥 DISCONNECTED 💥")
                elif not interface["enabled"]:
                    step.failed(f"{device} {interface_id} is ❌ DOWN ❌") 
                else:
                    step.passed(f"{device} {interface_id} is ✅ UP ✅")
...
if __name__ == "__main__":
    ...
    # Define a list or tuple of devices to be targeted using their serial number
    serials = os.getenv("MERAKI_SWITCH_SERIALS") or None
    if not serials:
        print("Did you export serials in the environment variables?")
        exit()
    else:
